  ...
  onMount(async () => {
    try {
      ...
      solicitacoes = todasSolicitacoes.filter((s: any) => 
        s.usfOrigem === 'USF04' && // Filtro específico para USF01
...
<div class="flex h-screen bg-gray-100">
  ...
  <div class="flex-1 flex flex-col">
    ...
    <header class="bg-emerald-700 text-white shadow p-4 flex items-center justify-between">
      <h1 class="text-xl font-semibold">Pacientes Pendentes - USF 04</h1>
          <UserMenu/>
    </header>
    ...
    <main class="flex-1 overflow-auto p-6">
      <div class="bg-white rounded-lg shadow-lg p-6 space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:justify-between md:items-center">
          <h2 class="text-2xl font-bold text-emerald-800 mb-4 md:mb-0">Lista de Pacientes Pendentes (USF 04)</h2>
        ...
        {:else}
            ...
            {#if filtradas.length === 0}
              <p class="text-center text-gray-500 py-10">
                ...
                {:else}
                    Nenhuma solicitação pendente para a USF 04 no momento.
                {/if}
              </p>
            {:else}
              <ul class="space-y-4">
